...
public:
    int maxFreeTime(int eventTime, vector<int>& startTime, vector<int>& endTime) {
       ...
       for(int i = 1; i < n; i++) {
            int currEventTime = endTime[i-1] - startTime[i-1]; // value of d 

            
        // case 1: completely move the element out of the place
           if(currEventTime <= max(maxLeftFree[i-1], maxRightFree[i])) {
                result = max(result, freeArray[i-1] + currEventTime + freeArray[i]);
            }

        // case 2: when we can move one event to right or left
